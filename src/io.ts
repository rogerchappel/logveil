...
export async function validateOutputDestinations(
  inputPaths: string[],
  ...
  for (const output of outputs) {
    ...
    if (previous) {
      throw new Error(`${output.flag} destination collides with ${previous}: ${output.path}`);
    }
    destinations.set(canonical, output.flag);
  }
}
