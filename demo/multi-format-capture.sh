...
OUT_DIR="${TMPDIR:-/tmp}/logveil-multi-format-demo"
...
node dist/cli.js audit examples --format json --fail-on none > "$AUDIT_OUT"

test -s "$MARKDOWN_OUT"
test -s "$JSON_OUT"
test -s "$AUDIT_OUT"
grep -q "REDACTED" "$MARKDOWN_OUT"
grep -q "examples/session.jsonl" "$MARKDOWN_OUT"
grep -q '"files": 3' "$JSON_OUT"
